...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        pe[:, 0::2] = torch.sin(position * div_term)    #(512,256)
        pe[:, 1::2] = torch.cos(position * div_term)
        pe = pe.unsqueeze(0).transpose(0, 1)  #(max_len,1,de_model)
    ...
    def forward(self, x):
        ...
        """
        x = x + self.pe[:x.size(0), :]   #将与句子长度对应位置向量加上
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.projection = nn.Linear(
            d_model, tgt_vocab_size, bias=True).to(device)
...
def greedy_decoder(model, enc_input, start_symbol):
    ...
    #     -1)
    greedy_dec_predict = dec_input[:, 0:]
